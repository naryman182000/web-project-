  ...
  if($_SERVER['REQUEST_METHOD'] == "POST")
	...
	#	$password = $_POST['password'];
		...
		if(!empty($location) &&!empty($distination) &&!empty($email) && !empty($hight)&& !empty($data)&& !empty($typeofgoods) && !is_numeric($email))
		{
			...
			$query = "insert into foodtrucks (location,distination,email,hight,data,typeofgoods,id) values ('$location','$distination','$email','$hight' ,'$data','$typeofgoods','$id')";
...
<body>
  ...
  <div id="user" aria-required="true">
    ...
    <form action="foodtrucks.php" method="post" >
      <br>
