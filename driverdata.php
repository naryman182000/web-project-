...
<body>
  ...
  <?php while($rows=mysqli_fetch_assoc($result)) 
        ...
        <td><?php echo $rows['licence_date']; ?></td> 
        <td><?php echo $rows['National_ID']; ?></td> 
        <td><?php echo $rows['gender']; ?></td> 
		<td><?php echo $rows['password']; ?></td> 
      
        <td><?php 
        if( $rows['active']==1){echo"<p id=str ".$rows['driver_id'].">Active</p>";}
        else{echo"<pp id=str ".$rows['driver_id'].">DeActive</p>";}
        ?></td> 
		
	
    <td>
    <select onchange="active_deactive_driver(this.value,<?php echo $rows['driver_id'];?>)">
    <option > </option>
    ...
    <td>
    <?php 
        if( $rows['type']==1){echo"<p id=strr ".$rows['driver_id'].">Admin</p>";}
        else{echo"<pp id=str ".$rows['driver_id'].">User</p>";}
        ?></td> 
    <td>
    <select onchange="admin_driver(this.value,<?php echo $rows['driver_id'];?>)">
    <option > </option>
...
<div id="userbtn">
   <a  href="driverform.php"  class="adduser-btn"> Add Driver </a>
  </div> 
